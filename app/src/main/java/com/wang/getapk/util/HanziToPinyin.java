...
public class HanziToPinyin {
    ...
    public String get(String input, boolean onlyFirstUpper){
        return get(input, null, onlyFirstUpper);
    }

    public String get(String input, String separator) {
        return get(input, separator, false);
    }
        ...
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                ...
                if (onlyFirstUpper && token.type == Token.PINYIN && token.target.length() > 1){
                    builder.append(token.target.charAt(0)).append(token.target.substring(1).toLowerCase());
                }else {
                    builder.append(token.target);
                }

                if (i != size - 1 && !TextUtils.isEmpty(separator)){
